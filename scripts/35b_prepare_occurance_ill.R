...
TEST_MODE <- FALSE
if(TEST_MODE){
  cat("[DEBUG] TEST MODE IS ACTIVATED, ONLY 5k CONTIGS SELECTED!!!\n")
}
...
coverage_dir <- "intermediate/occurance/mapping/illumina"
...
if (length(coverage_files) == 0) {
  ...
  for (file_path in coverage_files) {
    ...
    current_ill_id_from_file <- str_remove(file_name, pattern = "\\_mapped.*$")
    
    match_index <- match(current_ill_id_from_file, sample_names$ill)
    
    if (is.na(match_index) || length(match_index) == 0) {
      warning(paste0("No matching 'ill' ID found in 'sample_names' for file: ", file_name, ". Skipping."))
      next
    }
    ...
    if (length(sample_short_name) > 1) {
      warning(paste0("Multiple matching 'sample_short' names for '", current_ill_id_from_file, "'. Using the first: '", sample_short_name[1], "'."))
...
if(TEST_MODE) {
  ...
  set.seed(42)
  test_contigs <- sample(occurance_filtered$contig_id, size = 5000)
...
fwrite(edge_list_rmt, paste0("intermediate/network/occurance_ill_strat.csv"))
cat("[INFO] final edgelist saved to: ", paste0("intermediate/network/occurance_ill_strat.csv"))
cat("[TIME] Elapsed:", format(Sys.time() - start_time), "\n")
