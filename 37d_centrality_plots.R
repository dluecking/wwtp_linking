...
#   filter(type != "occurance_negative")
...
network_df <- network_df %>%
  mutate(
    ...
    log_betweeness = log10(betweeness + 1)
  )
...
deg_plot <- ggplot(network_df, aes(x = contig_type, y = log_degree, fill = contig_type)) +
  geom_boxplot() +
  geom_signif(
...
bet_plot <- ggplot(network_df, aes(x = contig_type, y = log_betweeness, fill = contig_type)) +
  geom_boxplot() +
  geom_signif(
    comparisons = list(c("lc", "lc_gv_connected"), c("lc", "gv")),
    map_signif_level = TRUE, 
...
ggsave(plot = p, file = "final/centrality_plots/centrality_all_layers_lc_vs_lc-gv-connected_REVIEW.png", 
       width = 7, height = 4)
ggsave(plot = p, file = "final/centrality_plots/centrality_all_layers_lc_vs_lc-gv-connected_REVIEW.pdf", 
       width = 7, height = 4)
ggsave(plot = p, file = "final/centrality_plots/centrality_all_layers_lc_vs_lc-gv-connected_REVIEW.svg", 
       width = 7, height = 4)
